...
const productId = urlParams.get('id'); // Ejemplo: "M1"

// Cargar los datos de los productos desde productos.json
fetch('../productos.json')
    .then(response => response.json())
    .then(data => {
        // Buscar el producto en la categoría "Monitores"
        const category = data["Monitores"];
        const product = category.find(item => item.id === productId);

        if (product) {
            // Actualizar los elementos de la página con los datos del monitor
            ...
            product.opciones.forEach(option => {
                ...
                select.id = option.label.toLowerCase(); // Ejemplo: "resolución", "tamaño"
            ...
            document.body.innerHTML = "<h1>Monitor no encontrado</h1>";
        }
    })
    .catch(error => {
