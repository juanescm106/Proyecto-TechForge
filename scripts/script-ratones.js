...
fetch('../productos.json')
    ...
    .then(data => {
        // Buscar el producto en la categoría "Ratones"
        const category = data["Ratones"];
        const product = category.find(item => item.id === productId);

        if (product) {
            // Actualizar los elementos de la página con los datos del ratón
            ...
            product.opciones.forEach(option => {
                ...
                select.id = option.label.toLowerCase(); // Ejemplo: "dpi", "color"
            ...
            document.body.innerHTML = "<h1>Ratón no encontrado</h1>";
        }
    })
    .catch(error => {
