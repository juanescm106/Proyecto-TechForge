...
const productId = urlParams.get('id'); // Ejemplo: "A1"

// Cargar los datos de los productos desde productos.json
fetch('../productos.json')
    .then(response => response.json())
    .then(data => {
        // Buscar el producto en la categoría "Auriculares"
        const category = data["Auriculares"];
        const product = category.find(item => item.id === productId);

        if (product) {
            // Actualizar los elementos de la página con los datos del auricular
            ...
            product.opciones.forEach(option => {
                ...
                select.id = option.label.toLowerCase(); // Ejemplo: "conectividad", "color"
            ...
            document.body.innerHTML = "<h1>Auricular no encontrado</h1>";
        }
    })
    .catch(error => {
